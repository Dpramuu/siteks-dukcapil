...
export default function TemplatePage({ initialTemplates }: Props) {
  // Inisialisasi dari data server — tidak ada useEffect fetch
  ...
  // ── CREATE / UPDATE ──
  const handleSave = async (formData: Omit<Template, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => {
    setApiError(null);

    if (editTarget) {
      // UPDATE — PATCH /api/templates/[id]
      ...
      if (!res.ok) {
        ...
        setApiError(err.error ?? 'Gagal mengupdate template.');
        return;
      }
      ...
      if (!res.ok) {
        const err = await res.json();
        setApiError(err.error ?? 'Gagal menyimpan template.');
        return;
      }

      const created: Template = await res.json();
      setTemplates(prev => [created, ...prev]); // terbaru di atas
    }
  ...
  const handleDelete = async () => {
    ...
    if (!res.ok) {
      const err = await res.json();
      setApiError(err.error ?? 'Gagal menghapus template.');
